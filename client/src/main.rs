...
fn read_cfg() -> Cfg {
    ...
    let feel_demo = std::env::var("VOXELFORGE_FEEL_DEMO").is_ok();
...
pub(crate) fn fly_camera(
    ...
    let (wish, run_pressed, jump_pressed_now) = if cfg.feel_demo {
        feel_demo_input(time.elapsed_secs(), flat_fwd)
    } else {
    ...
    if (target_pivot - orbit.smoothed_pivot).length() > player_tuning::CAM_SNAP_DIST {
        ...
        let pv = orbit.pivot_vel;
        orbit.smoothed_pivot += pv * spring_dt;
    }
...
/// `VOXELFORGE_FEEL_DEMO_FILM=<dir>` — capture a full-rate PNG sequence
/// (`frame_00000.png`, ...) across the whole feel-demo run instead of just the
/// named beats, so `ffmpeg -framerate FEEL_DEMO_FILM_FPS -i frame_%05d.png`
/// produces actual before/after footage rather than a slideshow of stills.
...
const FEEL_DEMO_FILM_FPS: f32 = 20.0;
...
fn feel_demo_shots(
    ...
    if let Some(dir) = feel_demo_film_dir() {
        if now >= state.next_film_at {
            let path = format!("{dir}/frame_{:05}.png", state.film_frames);
            commands
                .spawn(Screenshot::primary_window())
                .observe(save_to_disk(path));
            state.film_frames += 1;
            state.next_film_at = now + 1.0 / FEEL_DEMO_FILM_FPS;
        }
    }
