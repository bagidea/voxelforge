...
pub fn setup_hero(
    ...
    fill(&mut commands, &counter, 4, 11, 2, 3, 2, 6);
    // Hero bowl on the island top (y=3), centred ~ x7,z4 — the DOF focus point.
    bowl(&mut commands, &cube, &ceramic, 7, 3, 4);
    // A second bowl far off on the back counter (depth cue, blurs out in DOF).
    bowl(&mut commands, &cube, &ceramic, 6, 4, 13);
    ...
    commands.spawn((
        DirectionalLight {
            color: Color::srgb(1.0, 0.80, 0.52),
            illuminance: illum,
    ...
    let (focus, aperture) = cfg.dof.unwrap_or([10.0, 2.8]).into_tuple2();
    ...
    let exposure_ev = cfg.exposure.unwrap_or(9.8);
    ...
    let (g_temp, g_sat, g_contrast) = cfg.grade.unwrap_or([0.20, 1.32, 1.15]).into_tuple3();
    ...
    commands.spawn((
        ...
        AmbientLight {
            ...
            brightness: cfg.ambient.unwrap_or(3400.0),
            affects_lightmapped_meshes: false,
        },
        Exposure { ev100: exposure_ev },
        Tonemapping::AcesFitted,
        // P0 filmic grade — kills the residual blue-wash + restores saturation/grain
        // that AcesFitted flattens. contrast is applied per-section (shadows/mid/high)
        // so the whole tonal range crisps up, not just the mids.
        ColorGrading {
            ...
            shadows: ColorGradingSection { contrast: g_contrast, ..default() },
...
fn bowl(
    ...
    let put = |c: &mut Commands, x: i32, y: i32, z: i32| {
        c.spawn((
            Mesh3d(cube.clone()),
            MeshMaterial3d(mat.clone()),
            Transform::from_xyz(x as f32 + 0.5, y as f32 + 0.5, z as f32 + 0.5),
    ...
    for x in cx - 1..=cx + 1 {
        for z in cz - 1..=cz + 1 {
            put(commands, x, base_y, z);
        }
    }
    // rim ring 5×5, hollow centre (walls, height 1) => bowl silhouette
    let y = base_y + 1;
    for x in cx - 2..=cx + 2 {
        for z in cz - 2..=cz + 2 {
