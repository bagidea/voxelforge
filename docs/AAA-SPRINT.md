...
| Story | **rose** | `docs/story-bible.md`, `assets/story/act1.json` | **Done — proof:** the "`act1.json` not started" cell was stale (second stale cell this sprint, after Character). `assets/story/act1.json` (501 lines) landed in `d2ba784` — `git ls-files` tracked, working tree clean, no local edits. Content: 5 chained quests `q1_embers → q2_voice_in_stone → q3_gatekeeper → q4_what_walls_remember → q5_sigil_that_knew_you → null`, 12 dialogue nodes, 11 lore items, 9 regions, 3 NPCs, opening + `act_end`. Director-verified 2026-07-31 by running both validators myself: `validate.py` → `VALID`, 0 errors / 0 warnings; `serde_check.py` (new, Rose's own independent validator — mirrors every `quest.rs` struct field/type/`rename`/optionality **and** hard-codes the engine reads: `start_quest` init_journal, `on_spawn` trigger, `dlg_maren_gate` 4 lines + 5 choices with choice[4] advancing q3, `defeat`/`approach_entity`/`reach_zone` objectives, `gate_square` + `guard_post_east` bounds, q3→q4 unlock) → `0 field/type errors`, `0 engine-contract failures`. **Cross-lane note for sun:** `lore_items` + `act_end` deserialize fine but `quest.rs` has no system that reads them, and q4's `place_block` / `interact` objective kinds have no trigger handler (`check_*_triggers` covers only defeat/reach_zone/approach) — engine TODOs on the Quest lane, tracked in `act1-script.md` §11.4/§11.6/§11.8, not story-data gaps. |
...
| 3 | 2026-07-31 ~15:5x | Poppy's dodge i-frames + parry→riposte (`168d141`, `175ded6`), plus story/map/character housekeeping commits landed on `main` since pass #2 | **PASS** — `scripts/prove_combat.sh` exit 0, all 8 `COMBAT_*` assertions verified (STRAFE/HIT/HEAVY/KILL/DEATH/RESPAWN/WALK_DONE), 0 failures | **PASS** — `scripts/build_safe.sh build --bin voxelforge --target-dir target-int` EXIT=0 (`_build.log`: 0 lines matching `^error`, graded via `${PIPESTATUS[0]}`+`grep`, not `tail`), binary `target-int/debug/voxelforge.exe` (15:47:51) confirmed newer than every `client/src/*.rs` **at build-finish time** | **Overall verdict: FAIL — do not treat this pass as green.** Gate 3, `scripts/prove_knockback.sh`, exited 1: its own internal STALE_BINARY_GUARD tripped because `client/src/hero.rs` had been modified after the binary was built. I independently re-ran the STALE_BINARY_GUARD against the whole workspace (not just `client/src`) myself and confirmed it: `client/src/hero.rs` (15:51:19) and `client/src/shot_main.rs` (15:51:33) are both newer than `target-int/debug/voxelforge.exe` (15:47:51) — both uncommitted, both pixel's Look/beauty-shot lane (`docs/LANES.md` line 30), edited live while gates 1–2 were running. Not a lane-ownership violation, not something I touched — pixel was mid-edit during this pass's gate window. Practical effect: gates 1 and 2's PASS is only valid for the binary as it stood at 15:47:51; it no longer reflects current `hero.rs`/`shot_main.rs`. Re-run all 3 gates on the next fresh build once pixel's edit lands. Build PID verified by exact PID (713, not just process-name match) confirmed exited before grading. |
