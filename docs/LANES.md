...
|---|---|---|---|
| Player / scene / spawn | `client/src/scene.rs` | **poppy** | Spawn placement, campsite, the `SPAWN_GROUND` invariant. **Pending cross-lane edit (2026-08-16):** flamingo's `dress_act1_cast` (dresses quest-spawned NPCs via `characters.rs`/`equipment.rs`) is in the tree uncommitted — route through poppy before landing. |
...
| Dialogue UI | `client/src/dialogue_ui.rs` | **monanisa** | Reassigned to monanisa 2026-08-14 by Director (presentation UI, matches HUD lane below) — previously sun 2026-07-31. Rewritten off `bevy_egui` onto native Bevy UI `Node`s to match `hud.rs`'s look, so the old egui gotchas noted here no longer apply. Contract unchanged: reads `quest::DialogueState`, writes `quest::DialogueUiEvent` — does not edit `quest.rs`. |
| Look / beauty shots | `client/src/hero.rs`, `client/src/shot_main.rs`, `docs/golden-beauty-shot.md`, `docs/look-acceptance-rubric.md` | **pixel** | `hero.rs` is a beauty-shot scene only — it holds no controller code. **Pending cross-lane edits (2026-08-16):** flamingo's `VOXELFORGE_CHARSHOT` stage and monanisa's `VOXELFORGE_ENEMYSHOT` stage are in the tree uncommitted (they `#[path]`-pull `characters.rs`/`equipment.rs`/`enemies.rs`). |
| Levels / map data | `maps/`, `scripts/gen_edhari.py` | **shiba** | Authored levels and the generators behind them. |
| Look post stack | `client/src/look.rs`, `docs/look-bible.md`, `docs/look-tier-spec.md` | **poppy** | `LookPlugin` — the post stack the *playable* game wears (Bokeh DoF · SSAO · PCSS · TAA · VolumetricFog). Distinct from pixel's beauty-shot lane: `hero.rs` renders the reference, `look.rs` ships it. Retired lane: web/wasm parity (`scripts/web-verify.mjs`, `grade_web_parity.py`) — the client went native-only at `65b1d12`. **Closed 2026-08-14 (sky-dome / gate3 atmos lane):** the god-ray / `FogVolume` / SAT-ladder lane looped on unfalsifiable diff-masks and plate-bound thresholds without producing a CEO-visible look change; capture stills are parked under `docs/assets/gate3/_rose_*` and the lane is not pursued further. **Reassigned 2026-08-16:** the `look.rs` post-stack lane itself is now held by poppy (look regrade / `ev100` exposure probes); an in-tree WIP marker (`ev100: 8.6`) is explicitly marked "do not commit" and must be measured before landing. |
...
| Settings / options UI | `client/src/settings_menu.rs` | **monanisa** | `SettingsPlugin` + the `settings_closed` run-condition `main.rs` gates the fly camera on. Assigned 2026-07-31 — file was new and unowned. Same `bevy_egui` 0.41 gotchas as the dialogue-UI row. **Pending cross-lane edit (2026-08-16):** yamamoto's music/ambient slider split (adds `AudioSettings.music`) is in the tree uncommitted. |
| Combat HUD | `client/src/hud.rs` | **monanisa** | `HudPlugin` — Option A "Corner Minimal" (`docs/hud-design.md`). Standalone lane, only reads `combat::{Health,Stamina,LockOn}` + `quest::ObjectiveText`. Was never listed here despite the file's own header saying so; added 2026-08-14 alongside the dialogue-UI reassignment (dialogue_ui.rs now shares its colour tokens). |
...
| Enemy AI / behaviour | `client/src/enemy_ai.rs`, `client/src/enemy_ai_proof_main.rs`, `docs/enemy-behavior.md` | **rose** | New lane, 2026-08-14 by Director. Drives any body via `Archetype` (Swarm/Bruiser/Pouncer) — decoupled from `enemies.rs`'s kinds; no `crate::` refs, same isolation rule as the visuals lane. Motion + telegraphs only: damage/parry wiring stays with combat. Proof bin links enemies.rs+enemy_ai.rs only (`voxelforge_enemyai_proof`). |
| Enemy visuals | `client/src/enemies.rs`, `assets/enemies/**`, `docs/enemy-design.md` | **monanisa** | New lane, 2026-08-14 by Director (CEO "enemies should look scary" brief). Bevy-only, no `crate::` reference — same self-containment rule as `characters.rs`, for the same reason (renders inside the isolated `voxelforge_shot` bin without touching `combat.rs`). Proposes the swap into `combat::spawn_guard_husk`; does not apply it — that edit belongs to the combat lane. |
| Character cast + equipment | `client/src/characters.rs`, `client/src/equipment.rs`, `client/src/char_shot_main.rs` | **flamingo** | Act I cast bodies (base body + 6 equipment slots). Bevy-only, no `crate::` ref — renders inside `voxelforge_shot` via `#[path]`. `scene.rs`'s `dress_act1_cast` consumes it (that scene edit routes through the scene lane). |
| Enemy design shot bin | `client/src/enemy_shot_main.rs` | **monanisa** | Isolated proof bin linking `enemies.rs` only (`voxelforge_enemyshot`); same isolation rationale as `char_shot_main.rs`. |
| Cutscene / conversation camera | `client/src/cutscene.rs` | **unowned** | New shared file — consumed by `anim.rs` (conversation layer) and `main.rs` (beauty-tour cinematic). No owner assigned yet; ask the Director before the first edit. |
| Story bible | `docs/story-bible.md` | **rose (Story)** | Canon narrative spine; full three-act rewrite is in the tree uncommitted (2026-08-16). |
