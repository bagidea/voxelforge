...
`_claims.py` C2 was written to be the numeric test of exactly this claim — its own docstring says
"no split = the sun casts nothing onto the ground". **It came back unable to tell the scenes apart,
and it belongs on the record next to the evidence that does hold:**

| C2 grass-luminance bimodality (otsu-sep) | pinned `b361a9d` | working tree 03:50 |
|---|---|---|
| outdoor-noon (claimed: no cast shadow) | 0.718 | 0.746 |
| evening-raking (claimed: casts correctly) | 0.730 | 0.759 |
| night-firelit (claimed: no cast shadow) | **0.983** | **0.984** |

The two scenes on opposite sides of the claim are 0.012 apart, and the scene with the *least* sun
scores highest — night's "split" is the fire falloff, which the metric cannot distinguish from a
shadow edge. **C2 is not evidence for or against the blocker; it is a broken instrument for this
question** and no number from it is quoted anywhere else in this sheet.

The blocker stands on the two things that did measure it: the ground-plane luminance maps
(`_shadowmask.py` — scene 1's dark pixels cling to cube side faces, scene 2 has a dark band lying
across open ground) and the 200% crops on the sheet. A single-frame population statistic was the
wrong tool; separating "shadow edge" from "light falloff" needs the A/B re-shoot above.

## Since this sheet was cut

The plates are being re-shot on a loop. Since `b361a9d` the working tree has held **at least three
different plate sets**: 03:38 (measured in the first version of this table), 03:43, and the one on
disk now. A delta row is therefore a *stamped snapshot*, not a standing fact — read the stamp.

**Snapshot graded below** — working tree, mtime 2026-08-15 03:50:38–03:51:07:

| plate | md5 |
|---|---|
| `outdoor-noon_after.png` | `bb18a84f0b` |
| `evening-raking_after.png` | `672bbe78fd` |
| `night-firelit_after.png` | `64a6cca33d` |

| | on `b361a9d` (graded) | working tree 03:50 | |
|---|---|---|---|
| night G3 p05-L | 7.3% **FAIL** | 8.1% **PASS** | ✅ fixed |
| noon: cast shadow on ground | none | none | ➖ blocker stands |
| evening G6 | L 53.4 FAIL | L 53.7 FAIL | ➖ unchanged |
| noon R−B | +75.6 | +87.5 | ⚠️ warmer |
| noon: B clamped to 0 | 9.43% of frame | **18.70%** | ⚠️ doubled |
| evening: B clamped to 0 | 4.77% | 8.67% | ⚠️ doubled |

Reproduce the right-hand column with `LOOKV4_LIVE=1` — same scripts, working-tree plates:

```
LOOKV4_LIVE=1 python scripts/_flamingo_lookv4_axes.py     # R-B, B=0%
LOOKV4_LIVE=1 python scripts/_flamingo_lookv4_stage.py    # then grade_gate.py on _fl_lookv4/*-nohud2.png
```

If your md5s differ from the table above, you are on a **later shoot** — re-measure, do not
reconcile. Superseded numbers, kept so a reader of the earlier version is not left guessing: the
03:38 set read night G3 8.8%, noon R−B +87.1, noon B==0 19.20%, evening B==0 8.59%. That set is
gone from disk. Every verdict has held across all three shoots; only the magnitudes move.

So: **must-fix #1 below is already done — do not re-raise it.** The blocker is untouched. The new
thing worth a look is the blue channel: nearly a fifth of the noon frame now has B clamped to 0, and
...
`_look_v4_probe.py`, and `_stage.py` → `scripts/grade_gate.py`. **Every one of them resolves its
input through `_flamingo_lookv4_plates.plate()`** — pinned set by default, working tree only under
`LOOKV4_LIVE=1`, and each prints a banner naming which set it read, so no output from this toolchain
is ever ambiguous about its own source. Plates resolved by
